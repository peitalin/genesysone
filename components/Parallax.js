...
class Parallax extends React.Component {
  ...
  componentDidMount() {
    window.addEventListener('scroll', throttle(this.handleScroll, 30))
    window.addEventListener('resize', throttle(this.handleResize, 30))
    this.handleResize()
    this.setState({
      elems: {
  ...
  handleScroll = (event) => {
    ...
    let {
      mount7, mount6, mount6demand, mount5,
      mount4, mount4supply, mount3, mount2, mount1, mount1axes,
      logo1, logoButtons, parallaxBox1, particles,
      maintitle,
    } = this.state.elems
